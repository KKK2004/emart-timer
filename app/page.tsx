...
export default function Page() {
  ...
  function createNewCustomer() {
    if (!tenNguoiBam.trim()) {
      alert("Bạn cần nhập tên người bấm trước khi tạo mã khách.");
      return;
    }
    ...
    <main style={{ minHeight: "100vh", background: palette.bg, padding: 18, color: palette.text, fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif" }}>
      <section style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gap: 16 }}>
        <header style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 26 }}>Emart Timer - mã khách theo người bấm</h1>
            <p style={{ margin: "6px 0 0", color: palette.sub }}>Tạo mã khách ở mục 1 trước. Mỗi người bấm có dãy mã riêng: Cường → CUxx001, An → ANxx001, Bảo → BAxx001. Ghi chú phân biệt khách sẽ hiện ở danh sách chọn lại.</p>
          </div>
        ...
        <section style={cardStyle}>
          ...
          <p style={{ margin: "10px 0 0", color: palette.sub, fontSize: 13 }}>
            Mã khách đang hiển thị ở mục này sẽ được áp dụng cho tất cả thao tác bên dưới. Mỗi tên người bấm có chuỗi mã riêng; ghi chú ở mục 2.1 dùng để phân biệt khách trong danh sách chọn lại.
          </p>
